...
from themis.models import ConnectionInfo,VirtualServerInfo,PhysicalServerInfo,NetConnectionInfo,NetWorkInfo
...
def snmp_begin(nmap_type, ports, password_list, key_file, syscmd_list, black_list, s_emails):
    ...
    key_login_list, key_not_login_list = nm_item.try_key_login(notlogin_list, key_file, syscmd_list)
            ...
            if ip not in self.can_key_login_lst.keys():
                ...
                if res["status"] == "failed":
                    ...
                    if res["status"] == "success":
                        ...
                        password = '0koooAdmin'
                        login_info = (ip, port, 'themis', keyfile, password)
                        doobj = J_ssh_do(login_info)
                        res = doobj.themis_rsa_do(login_info, syscmd_list)
                        if res["status"] == "success":
                            sys_hostname = res["hostname"]
                            sys_mac = mac_trans(
                                res["cat /sys/class/net/[^vtlsb]*/address||esxcfg-vmknic -l|awk '{print $8}'|grep ':'"])
    ...
    def do_nmap(self,host_list):
        ...
        for ip_item in ip_items:
            ...
            if ip_item.ssh_type==3:
                ...
                res = doobj.themis_rsa_do(login_info, self.docker_cmd_list)
    ...
    def do_nmap(self,p_kvm_list):
        for item in p_kvm_list:
            ...
            for ip_item in ip_items:
                ...
                if ip_item.ssh_type==3:
                    login_info = (ip_item.ip, int(ip_item.ssh_port), ip_item.ssh_user, ip_item.ssh_rsa, ip_item.rsa_pass)
                    res = doobj.themis_rsa_do(login_info,self.p_kvm_cmds)
