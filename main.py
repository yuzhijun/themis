...
# import sys
os.environ["DJANGO_SETTINGS_MODULE"] = 'admin.settings.scan_hosts_settings'
import django
import time
django.setup()
...
from themis.models import  PhysicalServerInfo,ConnectionInfo,OtherMachineInfo,StatisticsRecord
from admin.settings.scan_hosts_settings import BASE_DIR
...
from scanhosts.models import MachineOperationsInfo
from themis.util.machines import Machines
...
def main():
    ...
    #             ob = OtherMachineInfo.objects.filter(sn_key=other_sn)
    #             if not ob:
    #                 OtherMachineInfo.objects.create(ip=item, sn_key=other_sn, reson_str=u"IP存活，非Linux服务器",
    #                                                 oth_cab_id=1)
    #
    # '''
    # 网络设备备份或者登录功能
    ...
    docker_p_list = p_relate_dic["docker-containe"]
    ...
    StatisticsRecord.objects.create(all_count=info_dic['all_c'], pyh_count=info_dic['pyh_c'],
                                    net_count=info_dic['net_c'], other_count=info_dic['other_c'], vmx_count=info_dic['vmx_c'], kvm_count=info_dic['kvm_c'], docker_count=info_dic['docker_c'])
